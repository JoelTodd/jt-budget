...
- Add inline comments sparingly, only where control flow, layout policy, persistence semantics, or other behavior would otherwise take time to infer.
...
- Use comments to reinforce project contracts when they are easy to violate, such as derived-value recomputation, strict sync behavior, or terminal layout constraints.
- If behavior is unclear, inspect the surrounding code and run the relevant workflow before commenting so the comment reflects real behavior rather than a guess.
